...
public class CustomerController {
   ...
   @RequestMapping("/find/{keyField}/{keyword}/{pgNum}")
    public @ResponseBody HashMap<String, Object> find(
         ...
         List<CustomerDTO> list = service.find(command);
         int[] pages = Pagination.getPages(list.size(), pgNum);
         int[] rows = Pagination.getRows(list.size(), pgNum, Values.PG_SIZE);
         command.setStart(rows[0]);
         command.setEnd(rows[1]);
         logger.info("{}", command.getStart());
         logger.info("{}", command.getEnd());
